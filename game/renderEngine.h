...
#define VkVerify(expr) {VkResult tmp = expr; if (tmp != VK_SUCCESS){ throw std::runtime_error(string_VkResult(tmp)); }}
...
public:
	...
	VkResult allocateMemory(VkMemoryPropertyFlags properties, ShaderBuffer* pBuffers, size_t bufferCount, VkDeviceMemory& memory);
	VkResult createBuffer(VkBufferUsageFlags usage, ShaderBuffer& buffer);
	VkImageView createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, VkDevice device, uint32_t mipLevels);
	void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory);	
	std::vector<char> readFile(const std::string& filename);
	VkResult copyBuffer(ShaderBuffer& srcBuffer, ShaderBuffer& dstBuffer, VkBufferCopy copyRegion);
...
private:
	...
	void beginRecordingCommandBuffer(VkCommandBuffer commandBuffer, uint32_t index);
	...
	VkFormat findSuppertedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features, VkPhysicalDevice device);
	uint32_t findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties);
	void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels, VkImageAspectFlags flags = VK_IMAGE_ASPECT_COLOR_BIT);
