...
int main() {                                // Anfang des Programms
    Game game;                              // erstellt Game
    auto lastTp = NOW;                      // setzt lastTp (letzter Zeitpunkt) auf jetzt 
    fSec diff = AS_SECONDS(1ms);            // setzt diff (Zeitdifferenz) auf eine millisekunde
    fSec desiredDiff = AS_SECONDS(17ms);    // setzt desiredDiff auf 17 millisekunden
                                            // desiredDiff = gewünschte Zeitdifferenz
    while (game.running()){                 // wiederholt eingerücktes bis das Spiel geschlossen wird
        game.update(diff.count());          // ruft die updatemethode von game mit diff in sekunden auf
