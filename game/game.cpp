...
Game::Game(){
	...
	glfwSetFramebufferSizeCallback(engine.window, [](GLFWwindow* win, int nWidth, int nHeight){
		reinterpret_cast<Game*>(glfwGetWindowUserPointer(win))->engine.framebufferSizeCallback(nWidth, nHeight);
	});
	
	glfwSetCursorPosCallback(engine.window, [](GLFWwindow* win, double _x, double _y){
	...
	glfwSetMouseButtonCallback(engine.window, [](GLFWwindow* win, int button, int action, int mods){
		Game* game = reinterpret_cast<Game*>(glfwGetWindowUserPointer(win));
...
Game::~Game(){
	...
	vkDestroyPipeline(device, bulletPipeline, nullptr);
...
void Game::update(float deltaTime){
    ...
    vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, bulletPipeline);

    vkCmdBindVertexBuffers(commandBuffer, 0, 1, &bulletVertexBuffer.buffer, &ZERO);
...
void Game::updatePlayer(float deltaTime){
	...
	if (keymap[GLFW_KEY_D]){
		player.position += glm::cross(player.forward, player.up) * SPEED * deltaTime;
	}
	...
	if (leftMouseButtonDown){
		...
		Projectile toPush;
		toPush.forward = player.forward * BULLET_SPEED + glm::vec3(dist(randomState), dist(randomState), dist(randomState));
		toPush.position = player.position + player.forward * (player.HURTBOX_RADIUS + 0.2f);
...
void Game::updateEnemies(float deltaTime){
	...
	if (enemies.size() < MAX_ENEMY_COUNT) {enemies.push_back(toPush);}
	
	enemies.erase(std::remove_if(enemies.begin(), enemies.end(), [](const Enemy& e){return e.health < 0.0f;}), enemies.end());
	...
	for (Enemy& e : enemies){
		...
		for (Enemy& other : enemies){
			if (other.position == e.position) {continue;}
			float distSq = glm::dot(other.position - e.position, other.position - e.position);
	...
	for (const Enemy& e : enemies){
		...
		glm::vec3 rotationAxis = glm::normalize(glm::cross({0.0f, 0.0f, -1.0f}, e.forward));
		float angle = glm::acos(glm::dot(e.forward, glm::vec3(0.0f, 0.0f, -1.0f)));
		toPush.transformMat = glm::rotate(glm::translate(glm::identity<glm::mat4>(), e.position), angle, rotationAxis);
...
void Game::updateBullets(float deltaTime){
	...
	const auto nBegin = std::remove_if(projectiles.begin(), projectiles.end(), [this](const Projectile& p){
		return glm::dot(player.position - p.position, player.position - p.position) > CUTOFF_DIST * CUTOFF_DIST;
	});
	...
	for (Projectile& p : projectiles){
		for (Enemy& e : enemies){
			...
			if (perp == glm::vec3(0.0f, 0.0f, 0.0f)){continue;}
	...
	projectiles.erase(std::remove_if(projectiles.begin(), projectiles.end(), [](const Projectile& p){ return p.forward == glm::vec3(0.0f, 0.0f, 0.0f); }), projectiles.end());
...
void Game::createGraphicsPipeline(){
    ...
    // pipeline layout
    {
		...
		VkVerify(vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &pipelineLayout));
    }
    ...
    // standard pipeline
	{
		...
		inputAssemblyInfo.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST; // never use primitive topology
		...
		VkVerify(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline))
	...
	// bullet pipeline
	{
		auto vertShaderCode = engine.readFile(SHADER_ROOT + "compiled/bulletVert.spv");
		VkShaderModule vertShaderModule = engine.createShaderModule(vertShaderCode);
		auto fragShaderCode = engine.readFile(SHADER_ROOT + "compiled/bulletFrag.spv");
		VkShaderModule fragShaderModule = engine.createShaderModule(fragShaderCode);
		...
		VkVerify(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &bulletPipeline))
...
void Game::loadModelData(){
	...
	VkVerify(engine.createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, vertexBuffer))	
	indexBuffer.size = eyeballIndices.size()*sizeof(eyeballIndices[0]);
	VkVerify(engine.createBuffer(VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, indexBuffer))
    
    bulletVertexBuffer.size = sizeof(Projectile)*MAX_INSTANCES;
    VkVerify(engine.createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, bulletVertexBuffer))
    
	std::array<ShaderBuffer, 3> buffers = {vertexBuffer, indexBuffer, bulletVertexBuffer};
    VkVerify(engine.allocateMemory(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, buffers.data(), buffers.size(), bufferMemory))
    
    ShaderBuffer stagingBuffer;
    stagingBuffer.size = std::max_element(buffers.begin(), buffers.end(), [](const ShaderBuffer& a, const ShaderBuffer& b){
...
void Game::createDescriptorSetLayouts(){
    // create the per frame set layout
	{
		...
		VkVerify(vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &globalDescriptorSetLayout))
	}
}
...
void Game::createUniformBuffers(){
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
		...
		VkVerify(engine.createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, uniformBuffers[i]))
	}
	
	VkVerify(engine.allocateMemory(
...
void Game::createSSBOs(){	
	enemyStorageBuffer.size = MAX_FRAMES_IN_FLIGHT * sizeof(EnemyStorageBufferStruct) * MAX_ENEMY_COUNT;
	
	VkVerify(engine.createBuffer(
...
void Game::loadModel(const char* modelPath, std::vector<Vertex>& vertices, std::vector<uint32_t>& indices){
