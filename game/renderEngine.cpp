...
std::array<const char* const, 0> VK_VALIDATION_LAYERS = {}; //{ "VK_LAYER_KHRONOS_validation" };
...
VkCommandBuffer RenderEngine::beginRendering()
{
	...
	swapchainImageResult = vkAcquireNextImageKHR(device, swapchain, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);
	if (swapchainImageResult == VK_ERROR_OUT_OF_DATE_KHR){
		recreateSwapchain();
		return VK_NULL_HANDLE;
	}
	else if (swapchainImageResult != VK_SUBOPTIMAL_KHR && swapchainImageResult != VK_SUCCESS){
...
void RenderEngine::endRendering(){
    ...
    std::array<VkSemaphore, 1> signalSemaphores = { renderFinishedSemaphores[imageIndex] };
...
bool RenderEngine::running() const
{
...
void RenderEngine::choosePhysicalDevice()
{
	...
	std::vector<VkPhysicalDevice> devices; devices.resize(physicalDeviceCount);
...
void RenderEngine::setupDebugMessenger()
{
	...
	if (func) {
		...
		throw std::runtime_error("failed to retrieve debug messenger creation function");
	}
}
...
void RenderEngine::createSwapchain()
{
	...
	if (imageCount > supportDetails.capabilities.maxImageCount && supportDetails.capabilities.maxImageCount > 0){
	...
	zImageView = createImageView(zImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, device, 1);

	std::array<uint32_t, 2>(queueFamilyIndices) = { queueIndices.graphicsQueue.value(), queueIndices.presentQueue.value() };
	...
	VkVerify(vkGetSwapchainImagesKHR(device, swapchain, &imageCount, nullptr))
	...
	VkVerify(vkGetSwapchainImagesKHR(device, swapchain, &imageCount, swapchainImages.data()))
	...
	for (size_t i = 0; i < imageCount; ++i) {
		...
		VkVerify(vkCreateFence(device, &fenceInfo, nullptr, &inFlightFences[i]))
	}
}
...
void RenderEngine::beginRecordingCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex){	
...
VkSurfaceFormatKHR RenderEngine::chooseSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats)
{
...
VkPresentModeKHR RenderEngine::choosePresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes)
{
...
bool RenderEngine::isDeviceSuitable(VkPhysicalDevice device)
{
	...
	return 
		indices.complete() && extensionsSupported && swapChainAdequate && supportedFeatures.samplerAnisotropy && 
...
VkImageView RenderEngine::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, VkDevice device, uint32_t mipLevels)
{
...
VkFormat RenderEngine::findSuppertedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features, VkPhysicalDevice device)
{
...
void RenderEngine::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
{
...
void RenderEngine::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels, VkImageAspectFlags flags)
{
...
VkResult RenderEngine::allocateMemory(VkMemoryPropertyFlags properties, ShaderBuffer* pBuffers, size_t bufferCount, VkDeviceMemory& memory){
	...
	for (size_t i=0; i < bufferCount; ++i){
		...
		if (bufferOffsetAccumulator % requirements.alignment != 0){
			bufferOffsetAccumulator = (bufferOffsetAccumulator / requirements.alignment + 1) * requirements.alignment;
		}
	...
	VkMemoryAllocateInfo allocInfo{};
	allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
	allocInfo.allocationSize = pBuffers[bufferCount-1].offset + requirements.size;
	...
	for (size_t i=0; i < bufferCount; ++i){
		...
		result = vkBindBufferMemory(device, pBuffers[i].buffer, memory, pBuffers[i].offset);
		if (result != VK_SUCCESS){
			return result;
		}
	}
...
VkResult RenderEngine::copyBuffer(ShaderBuffer& srcBuffer, ShaderBuffer& dstBuffer, VkBufferCopy copyRegion){
	VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
VkResult RenderEngine::createStagingBuffer(ShaderBuffer& buffer, void** ppData){
